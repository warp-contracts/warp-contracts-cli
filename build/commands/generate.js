...
const generatePrompt = (templates, uiTemplates, load, options) => {
    let createProject = (projectName, projectChoice, appChoice) => {
        ...
        (0, child_process_1.exec)(appChoice !== 'undefined'
            ...
            : `mkdir ${projectName} && cd ${projectName} && git init && git remote add -f origin https://github.com/warp-contracts/templates && git config core.sparseCheckout true && echo "${projectChoice}" >> .git/info/sparse-checkout && git pull origin main && rm -rf .git && cd contracts && mv ${projectChoice}/* .`, (error, stdout, stderr) => {
            if (error) {
